...
产品方案被认可后，后续设计输出不是必选项。`Z-PRD` 会先判断是否值得继续；如果用户选择某种交付方式，会直接按对应平台输出，不只是停在选择题。
...
- 墨刀：页面结构、流程连线、交互说明、状态矩阵、可上传原型清单。
- 蓝湖：先询问是否有团队标准样式，再输出标注口径、组件清单、资产清单和研发验收点。
- Figma / MasterGo / Pixso：输出 frame 规划、组件映射、设计 token、prototype links 和 Dev Mode 说明。
- HTML/React：输出可运行原型、样例数据、交互覆盖和视觉验证清单。
